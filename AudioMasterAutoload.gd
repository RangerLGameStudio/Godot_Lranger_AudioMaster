...
#
#func ReturnSFXPlayerToQueue(audioPlayer):
	#available_SFXPlayer.playing=false
	#available_SFXPlayer.append(audioPlayer)
	#
	#
#func _process(delta):
	## Play a queued sound if any players are available.
	#if not SFXqueue.size()==0 and not available_SFXPlayer.empty():
		#var SFXRequest:AudioSFXFXRequest = SFXqueue.pop_front()
		#available_SFXPlayer[0].stream = load(SFXRequest.AudioPath)
		#available_SFXPlayer[0].volume_db = SFXRequest.GetVolume()
		#available_SFXPlayer[0].pitch = SFXRequest.GetPitch()
		#if SFXRequest.RemovePreviousAudio.size()>0:
			#for audioname in SFXRequest.RemovePreviousAudio:
				#SearchAndRemoveSFX(audioname)
		#available_SFXPlayer[0].play()
		#SFXPlayerToRequestDict[available_SFXPlayer[0]] = SFXRequest
		#available_SFXPlayer.pop_front()
